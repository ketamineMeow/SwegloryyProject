    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            while (true)
            {
                ...
                if (choose == 10)
                {
                    Console.Write("Задание 10 (23 вариант):Разработайте программу, которая принимает текст и выводит два слова: наиболее часто встречающееся и самое длинное. \n");
                    ...
                    var mostFrequentWord = wordCounts.OrderByDescending(w => w.Value).FirstOrDefault().Key;

                    var longestWord = words.OrderByDescending(w => w.Length).FirstOrDefault();

                    Console.WriteLine($" Наиболее частое слово: {mostFrequentWord}");
